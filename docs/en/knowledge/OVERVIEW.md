...
- **[02 — Clinical speech-to-text (Swedish)](02-clinical-stt-swedish.md)**: Wall-E vs Claude Pro for transcribing patient speech, the benchmark (WER, term recall, PII leaks), the Claude Pro capture protocol, the loop that optimises the pipeline against it, and the `wall-e/sv-medical` model that serves it through `/v1/chat/completions`.
